...
class pos_config(models.Model):
    ...
    display_delivery = fields.Boolean("Display Delivery Button", help="If Display Delivery Button is true than pos shows a delivery button.", default=True)
    display_parcel = fields.Boolean("Display Parcel Button", help="If Display Parcel Button is true than pos shows a parcel button.", default=True)
...
class PosOrder(models.Model):
    ...
    table_name = fields.Char(compute='get_table_name',string='Parcel Name', store=True)
                ...
                if not order.split_order and not order.folio_id :
                    for res_table in order.reserved_table_ids:
                        if (res_table.table_id.available_capacities - res_table.reserver_seat) == 0:
                            table_obj.browse(res_table.table_id.id).write({'state' : 'available', 'available_capacities' : res_table.reserver_seat - res_table.table_id.available_capacities })
                        else:
                            if (res_table.table_id.available_capacities - res_table.reserver_seat) > 0:
                                table_obj.browse(res_table.table_id.id).write({'state' : 'available', 'available_capacities': res_table.table_id.available_capacities - res_table.reserver_seat })
            except AttributeError:
    ...
    def _order_fields(self, ui_order):
        ...
        if ui_order.get('id', False) and table_data:
            self.browse(ui_order.get('id')).write({'reserved_table_ids': [(5, 0)]})
    ...
    def check_group_pos_delivery_boy(self, puser):
        ...
        grp_result = mod_obj.get_object('pos_order_for_restaurant', 'group_pos_delivery_boy')
        user_add = [user.id for user in grp_result.users]
        for user in mod_obj.get_object('point_of_sale', 'group_pos_manager').users:
    ...
    def close_order(self, order_id):
        ...
        is_kitchen_screen = ir_module_module_object.search([('state', '=', 'installed'), ('name', '=', 'pos_receipt')])
        line_ids = []
        ...
        if booked_table and booked_table.split( '_' ):
            for booked in booked_table.split( '_' ):
                if booked:
                    ...
                    table_data.write({'available_capacities': table_data.available_capacities - int( qty ), 'state': 'available'})
        return True
    ...
    def remove_order( self, second_order_id = False):
        if self.sudo().ids and second_order_id:
            ...
            self.env["pos.order.line"].browse(line_ids).write({"order_id" : ids[0]})
